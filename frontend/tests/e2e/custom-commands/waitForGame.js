...
const WaitFor = require('./_waitFor');

/**
 * Waits a given time in milliseconds for a the Phaser Game to be present in the page before performing any other commands or assertions.
 *
 ...
 *   browser.waitForGame(1000);
 *   // continue if failed
 *   browser.waitForGame(1000, false);
 *   // with callback
 *   browser.waitForGame(1000, function() {
 *     // do something while we're here
 *   });
 *   // many combinations possible - the message is always the last argument
 *   browser.waitForGame(1000, false, function() {}, 'Juego de phaser no era presente en %d ms');
 * };
 * ```
 *
 * @method waitForGame
...
WaitForGame.prototype.phaserComponentFound = function(result, now) {
  const defaultMsg = 'Game was found after %d milliseconds.';
...
WaitForGame.prototype.phaserComponentNotFound = function(result, now) {
  ...
  const defaultMsg = 'Timed out while waiting for Game for %d milliseconds.';
  return this.fail({value:false}, 'not found', this.expectedValue, defaultMsg, now - this.startTimer);
};

/**
 * Checks that Phaser Game is available, and calls the callback if it is.
 *
...
WaitForGame.prototype.getProtocolCommand = function (callback) {
  ...
  return this.api.execute(
    function () {
      return window.Phaser && window.Phaser.GAMES && window.Phaser.GAMES.length > 0;
    },
    [],
    function (result) {
