...
WaitForState.prototype.phaserComponentFound = function(result, now) {
  const defaultMsg = 'Phaser StateManager reached state: "%s" after %d milliseconds.';
...
WaitForState.prototype.phaserComponentNotFound = function(result, now) {
  ...
  const defaultMsg = 'Timed out while waiting for state: "%s" after %d milliseconds.';
...
WaitForState.prototype.getProtocolCommand = function (callback) {
  const self = this;
  const desiredState = this.selector;
  return this.api.execute(
    function (desiredState) {
      return window.Phaser && typeof window.Phaser.GAMES[0].state.current === 'string' && window.Phaser.GAMES[0].state.current.toString() === desiredState;
    },
    [desiredState],
    function (result) {
