...
WaitForGame.prototype.phaserComponentFound = function(result, now) {
  const defaultMsg = 'E2E Module was found after %d milliseconds.';
...
WaitForGame.prototype.phaserComponentNotFound = function(result, now) {
  ...
  const defaultMsg = 'Timed out while waiting for E2E Module for %d milliseconds.';
...
WaitForGame.prototype.getProtocolCommand = function (callback) {
  ...
  return this.execute(
    function () {
      return window.Phaser && typeof window.Phaser.GAMES[0].e2e !== 'undefined';
    },
    [],
    function (result) {
